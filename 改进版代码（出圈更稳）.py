...
BASE_COMMAND = 971
MAX_FORWARD_LEFT = 973
MAX_FORWARD_RIGHT = 973
...
CLOSED_LOOP_TARGET_STEP_RPM = 180.0
...
SPEED_FILTER_ALPHA = 0.45
SPEED_TARGET_DROP_RESET_RPM = 16.0
SPEED_OVERSPEED_DECAY_RPM = 18.0
SPEED_OVERSPEED_INTEGRAL_DECAY = 0.45
...
# 大圆圈是顺时针连续右弯。专用控制只在两个十字之间启用：
# 对离散的五路误差做低通，保留连续右弯前馈，并限制瞬时转向幅度，
# 避免“直走一下－猛右转一下”的周期性抽动。
CIRCLE_PID_KP = 0.300
CIRCLE_PID_KI = 0.0
CIRCLE_PID_KD = 0.08
CIRCLE_ERROR_FILTER_ALPHA = 0.24
CIRCLE_DERIVATIVE_FILTER_ALPHA = 0.25
CIRCLE_COMMAND_FILTER_ALPHA = 0.30
CIRCLE_RIGHT_TURN_BIAS = 160.0
CIRCLE_BIAS_FILTER_ALPHA = 0.16
CIRCLE_MAX_RIGHT_TURN = 430.0
CIRCLE_MAX_LEFT_CORRECTION = 260.0
...
POST_CURVE_ENTER_ERROR = 480.0
POST_CURVE_EXIT_ERROR = 180.0
POST_CURVE_DAMP_MIN_MS = 110
POST_CURVE_DAMP_MAX_MS = 240
POST_CURVE_STABLE_ERROR = 130.0
POST_CURVE_STABLE_FRAMES = 5
POST_CURVE_BASE_COMMAND = 850
POST_CURVE_TURN_SCALE = 1.00
POST_CURVE_DERIVATIVE_SCALE = 0.35
POST_CURVE_MAX_TURN = 240.0
...
FIRST_CROSS_RIGHT_BIAS = 100.0
FIRST_CROSS_BIAS_MAX_ERROR = 260.0
FIRST_CROSS_BIAS_LEFT_TOLERANCE = 40.0
CROSS_ENTRY_FILTER_ALPHA = 0.28
CROSS_ENTRY_LIVE_BLEND = 0.30
CROSS_COOLDOWN_MS = 70
CROSS_SUM_MIN = 1850
CROSS_SIDE_SUM_MIN = 520
CROSS_SKEW_OPPOSITE_MIN = 210
CROSS_SKEW_OPPOSITE_SUM_MIN = 260
CROSS_APPROACH_GUARD_MS = 22
CROSS_APPROACH_COMMAND = 860
CROSS_APPROACH_REARM_FRAMES = 2
...
def is_circle_exit_cross(cross_count):
    return cross_count == 1 or cross_count == 3


def first_cross_bias(cross_count, entry_error=0.0):
    # 顺时针进入大圆圈时保留原有轻微右偏，但小车尚未回正时
    # 优先按实际误差回线，避免固定右偏盖过左向纠偏。
    if (
        is_circle_entry_cross(cross_count) and
        entry_error >= -FIRST_CROSS_BIAS_LEFT_TOLERANCE and
        entry_error <= FIRST_CROSS_BIAS_MAX_ERROR
    ):
...
def cross_entry_error(position):
    if abs(last_error) >= 50.0:
        ...
        live_error = position - LINE_CENTER
    else:
        live_error = 0.0

    if cross_entry_error_ready:
        entry_error = (
            (1.0 - CROSS_ENTRY_LIVE_BLEND) * cross_entry_error_filtered +
            CROSS_ENTRY_LIVE_BLEND * live_error
        )
    else:
        entry_error = live_error
...
cross_entry_error_filtered = 0.0
cross_entry_error_ready = False

circle_error_filtered = 0.0
circle_last_filtered_error = 0.0
circle_derivative_filtered = 0.0
circle_turn_bias = 0.0
circle_filter_ready = False

post_curve_armed = False
post_curve_damp_start_ms = ticks_ms() - POST_CURVE_DAMP_MAX_MS
post_curve_stable_frames = 0
...
def reset_cross_entry_error_filter():
    global cross_entry_error_filtered
    global cross_entry_error_ready

    cross_entry_error_filtered = 0.0
    cross_entry_error_ready = False


def update_cross_entry_error_filter(error):
    global cross_entry_error_filtered
    global cross_entry_error_ready

    if not cross_entry_error_ready:
        cross_entry_error_filtered = error
        cross_entry_error_ready = True
    else:
        cross_entry_error_filtered += CROSS_ENTRY_FILTER_ALPHA * (
            error - cross_entry_error_filtered
        )


def reset_circle_pid_filter():
    global circle_error_filtered
    global circle_last_filtered_error
    global circle_derivative_filtered
    global circle_turn_bias
    global circle_filter_ready
...
def update_circle_pid_filter(error):
    global circle_error_filtered
    global circle_last_filtered_error
    global circle_derivative_filtered
    global circle_turn_bias
    ...
    if not circle_filter_ready:
        circle_error_filtered = error
        circle_last_filtered_error = error
        circle_derivative_filtered = 0.0
        circle_filter_ready = True
    else:
        circle_error_filtered += CIRCLE_ERROR_FILTER_ALPHA * (
            error - circle_error_filtered
        )
        raw_derivative = circle_error_filtered - circle_last_filtered_error
        circle_derivative_filtered += CIRCLE_DERIVATIVE_FILTER_ALPHA * (
            raw_derivative - circle_derivative_filtered
        )
        circle_last_filtered_error = circle_error_filtered

    circle_turn_bias += CIRCLE_BIAS_FILTER_ALPHA * (
        CIRCLE_RIGHT_TURN_BIAS - circle_turn_bias
    )
    return (
        circle_error_filtered,
        circle_derivative_filtered,
        circle_turn_bias
    )
...
def reset_post_curve_damp():
    ...
    global post_curve_stable_frames
...
def update_post_curve_damp(error, now):
    ...
    damp_elapsed = ticks_diff(now, post_curve_damp_start_ms)
    if damp_elapsed < 0 or damp_elapsed >= POST_CURVE_DAMP_MAX_MS:
        post_curve_stable_frames = 0
        return False

    if abs_error <= POST_CURVE_STABLE_ERROR:
        post_curve_stable_frames += 1
    else:
        post_curve_stable_frames = 0

    if (
        damp_elapsed >= POST_CURVE_DAMP_MIN_MS and
        post_curve_stable_frames >= POST_CURVE_STABLE_FRAMES
    ):
        post_curve_damp_start_ms = now - POST_CURVE_DAMP_MAX_MS
        post_curve_stable_frames = 0
        return False
...
def reset_pid_history():
    ...
    last_left_command = 0.0
    last_right_command = 0.0
    reset_cross_entry_error_filter()
    reset_circle_pid_filter()
...
def follow_line_step(base_command, circle_pid_active=False):
    global pid_integral
    global last_error
    global last_left_command
    global last_right_command
    ...
    update_cross_entry_error_filter(error)

    if circle_pid_active:
        # 大圆圈是连续右弯，不把传感器从中路跳到右路的离散变化
        # 直接送入 PID，也不反复触发普通弯道的“出弯防抖”。
        pid_integral = 0.0
        control_error, derivative, circle_bias = update_circle_pid_filter(error)
        post_curve_damp_active = False
        active_kp = CIRCLE_PID_KP
        active_ki = CIRCLE_PID_KI
        active_kd = CIRCLE_PID_KD
    else:
    ...
    turn = (
        active_kp * control_error +
        active_ki * pid_integral +
        active_kd * derivative +
        circle_bias
    )
    tracking_base_command = curve_base_command(base_command, control_error)

    if circle_pid_active:
        turn = clamp(
            turn,
            -CIRCLE_MAX_LEFT_CORRECTION,
            CIRCLE_MAX_RIGHT_TURN
        )
    ...
    command_filter_alpha = (
        CIRCLE_COMMAND_FILTER_ALPHA if circle_pid_active
        else COMMAND_FILTER_ALPHA
    )
    if (
        circle_pid_active and
        last_left_command == 0.0 and
        last_right_command == 0.0
    ):
        # 十字保持结束后 PID 历史为零，首帧直接接上目标，
        # 避免专用低通从零慢慢爬升造成一次无意义的顿挫。
        filtered_left = left
    ...
    left = clamp(filtered_left, -MAX_REVERSE_LEFT, MAX_FORWARD_LEFT)
    right = clamp(filtered_right, -MAX_REVERSE_RIGHT, MAX_FORWARD_RIGHT)
...
def track_loop():
    ...
    cross_approach_consumed = False
    cross_approach_clear_frames = 0

    # 大圆圈 PID 状态：进入十字完成后启用，离开十字识别时关闭。
    circle_pid_active = False
    circle_pid_pending = False
    ...
    while True:
        ...
        if cross_hold_active:
            if ticks_diff(now, cross_hold_start) < CROSS_HOLD_MS:
                ...
                if circle_pid_pending:
                    circle_pid_active = True
                    circle_pid_pending = False
                reset_pid_history()
            continue
        ...
        if detected:
            ...
            if not cross_cooldown_active and is_cross_detected():
                ...
                circle_exit_now = is_circle_exit_cross(cross_count)
                if circle_entry_now:
                    circle_pid_pending = True
                elif circle_exit_now:
                    circle_pid_active = False
                    circle_pid_pending = False
                ...
                cross_hold_bias = first_cross_bias(cross_count, cross_hold_error)
            ...
            # 圆圈内右侧两路同时压线是连续右弯的正常形态，不能把它
            # 当作“接近十字”。出圈仍由优先级更高的完整十字判据识别。
            cross_approach_now = (
                not circle_pid_active and
            ...
            if not cross_approach_now and not cross_approach_tail_now:
                cross_approach_clear_frames += 1
                if (
                    not cross_approach_active and
                    cross_approach_clear_frames >= CROSS_APPROACH_REARM_FRAMES
                ):
                    cross_approach_consumed = False
            else:
                cross_approach_clear_frames = 0

            cross_approach_can_start = (
                cross_approach_now and
                not cross_approach_active and
                not cross_approach_consumed
            )
            if (
                not cross_cooldown_active and
                (cross_approach_active or cross_approach_can_start)
            ):
                if cross_approach_can_start:
                    cross_approach_active = True
                    cross_approach_consumed = True
                ...
                if (
                    ...
                    corner_candidate_frames = 0
                    reset_pid_history()
                    left_command, right_command = cross_approach_commands(
                        cross_approach_error,
                        first_cross_bias(cross_count, cross_approach_error)
                    )
                    set_wheel_commands(
                        left_command,
                        right_command,
            ...
            # “接近十字”只允许连续触发一次保护窗。若保护窗结束后
            # 宽线形态仍未发展成十字，就立即允许直角判断，避免在
            # 直角入口反复重启 22 ms 的直行保护而迟迟不转。
            block_cross_approach_corner = not cross_approach_consumed

            if circle_pid_active:
                # 大圆圈是连续右弯，圆圈状态内只由专用 PID 控制，
                # 不允许右侧两路短暂同时压线触发直角强转。
                corner_direction_candidate = 0
            else:
            ...
            if circle_pid_active:
                state = "CIRCLE_PID"
            left_command, right_command, error = follow_line_step(
                base_command,
                circle_pid_active=circle_pid_active
            )
            set_wheel_commands(left_command, right_command)
